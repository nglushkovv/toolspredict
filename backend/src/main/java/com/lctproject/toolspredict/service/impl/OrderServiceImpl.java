...
public class OrderServiceImpl implements OrderService {
    ...
    public void createOrder(OrderRequest orderRequest) {
        try {
            ...
            for (Long toolId: orderRequest.getToolsList()) {
                try {
                    toolService.addToolOrderItem(toolId, order);
                } catch (NoSuchElementException e) {
                    log.error("Не зафиксирован инструмент под номером {}: нет в справочнике.", toolId);
                }
            }
        } catch (Exception e) {
